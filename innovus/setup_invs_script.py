...
if insert_lineno != -1:
	...
	template_lines[insert_lineno+6] = 'set bottom_routing_layer '+inputs[0]+'\n'
	template_lines[insert_lineno+7] = 'set top_routing_layer '+inputs[1]+'\n'

	template_lines[insert_lineno+8] = 'set lef "../library/NangateOpenCellLibrary.tech.lef"\n'

	#If design has no clock, turn off CTS
	if has_clk == "False":
		for lineno, line in enumerate(template_lines):
			if line.startswith("ccopt_design"):
				template_lines[lineno] = "# ccopt_design\n"
				break
